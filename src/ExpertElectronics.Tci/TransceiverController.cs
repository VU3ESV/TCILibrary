...
public class TransceiverController : ITransceiverController, IDisposable
{
    ...
    /// <summary>
    /// Enables or disables the CW terminal mode.
    /// </summary>
    public async Task SetCwTerminalMode(bool enable)
    {
        await TciClient.SendMessageAsync($"cw_terminal:{(enable ? "true" : "false")};");
    }

    // ---------- TCI v2.0 extended state & commands ----------

    public uint CwKeyerSpeed
    {
        get => _cwKeyerSpeed;
        set
        {
            _cwKeyerSpeed = value;
            OnCwKeyerSpeedChanged?.Invoke(this, new UintValueChangedEventArgs(_cwKeyerSpeed));
        }
    }

    public int MonVolume
    {
        get => _monVolume;
        set
        {
            _monVolume = value;
            OnMonVolumeChanged?.Invoke(this, new IntValueChangedEventArgs(_monVolume));
        }
    }
    ...
    public bool MonEnable
    {
        ...
        {
            _monEnable = value;
            OnMonEnableChanged?.Invoke(this, new StateChangeEventArgs(_monEnable));
        }
    }
    ...
    public int DiglOffset
    {
        ...
        {
            _diglOffset = value;
            OnDiglOffsetChanged?.Invoke(this, new IntValueChangedEventArgs(_diglOffset));
        }
    }
    ...
    public int DiguOffset
    {
        ...
        {
            _diguOffset = value;
            OnDiguOffsetChanged?.Invoke(this, new IntValueChangedEventArgs(_diguOffset));
        }
    }

    public string AudioStreamSampleType
    {
        get => _audioStreamSampleType;
        set
        {
            _audioStreamSampleType = value;
            OnAudioStreamSampleTypeChanged?.Invoke(this, new StringValueChangedEventArgs(_audioStreamSampleType));
        }
    }

    public uint AudioStreamChannels
    {
        get => _audioStreamChannels;
        set
        {
            _audioStreamChannels = value;
            OnAudioStreamChannelsChanged?.Invoke(this, new UintValueChangedEventArgs(_audioStreamChannels));
        }
    }

    public uint AudioStreamSamples
    {
        get => _audioStreamSamples;
        set
        {
            _audioStreamSamples = value;
            OnAudioStreamSamplesChanged?.Invoke(this, new UintValueChangedEventArgs(_audioStreamSamples));
        }
    }

    public uint TxStreamAudioBuffering
    {
        get => _txStreamAudioBuffering;
        set
        {
            _txStreamAudioBuffering = value;
            OnTxStreamAudioBufferingChanged?.Invoke(this, new UintValueChangedEventArgs(_txStreamAudioBuffering));
        }
    }

    public bool RxSensorsEnable
    {
        get => _rxSensorsEnable;
        set
        {
            _rxSensorsEnable = value;
            OnRxSensorsEnableChanged?.Invoke(this, new StateChangeEventArgs(_rxSensorsEnable));
        }
    }

    public bool TxSensorsEnable
    {
        get => _txSensorsEnable;
        set
        {
            _txSensorsEnable = value;
            OnTxSensorsEnableChanged?.Invoke(this, new StateChangeEventArgs(_txSensorsEnable));
        }
    }
    ...
    public bool AppFocus
    {
        ...
        {
            _appFocus = value;
            OnAppFocusChanged?.Invoke(this, new StateChangeEventArgs(_appFocus));
        }
    }
    ...
    public long TxFrequency
    {
        ...
        {
            _txFrequency = value;
            OnTxFrequencyChanged?.Invoke(this, new LongValueChangedEventArgs(_txFrequency));
        }
    }

    /// <summary>
    /// Sets the CW keyer speed in WPM (1..60).
    /// </summary>
    public async Task SetCwKeyerSpeed(uint wpm)
    {
        if (wpm < 1 || wpm > 60) return;
        if (CwKeyerSpeed == wpm) return;
        await TciClient.SendMessageAsync($"{TciCwKeyerSpeedCommand.Name}:{wpm};");
    }

    /// <summary>
    /// Sets the global TX-monitor volume in dB (-60..0).
    /// </summary>
    public async Task SetMonVolume(int valueInDb)
    {
        if (valueInDb < -60 || valueInDb > 0) return;
        if (MonVolume == valueInDb) return;
        await TciClient.SendMessageAsync($"{TciMonVolumeCommand.Name}:{valueInDb};");
    }

    public async Task SetMonEnable(bool state)
    {
        if (MonEnable == state) return;
        await TciClient.SendMessageAsync($"{TciMonEnableCommand.Name}:{(state ? "true" : "false")};");
    }

    public async Task SetAgcMode(uint transceiverPeriodicNumber, string mode)
    {
        var trx = GetTransceiver(transceiverPeriodicNumber);
        if (trx == null) return;
        if (trx.AgcMode == mode) return;
        await TciClient.SendMessageAsync($"{TciAgcModeCommand.Name}:{transceiverPeriodicNumber},{mode};");
    }

    public async Task SetAgcGain(uint transceiverPeriodicNumber, int gainDb)
    {
        var trx = GetTransceiver(transceiverPeriodicNumber);
        if (trx == null) return;
        if (gainDb < -20 || gainDb > 120) return;
        if (trx.AgcGain == gainDb) return;
        await TciClient.SendMessageAsync($"{TciAgcGainCommand.Name}:{transceiverPeriodicNumber},{gainDb};");
    }

    public async Task RxNbEnable(uint transceiverPeriodicNumber, bool state)
    {
        var trx = GetTransceiver(transceiverPeriodicNumber);
        if (trx == null) return;
        if (trx.RxNb == state) return;
        await TciClient.SendMessageAsync($"{TciRxNbEnableCommand.Name}:{transceiverPeriodicNumber},{(state ? "true" : "false")};");
    }

    public async Task RxNbParam(uint transceiverPeriodicNumber, int threshold, int pulseDurationUs)
    {
        if (threshold < 1 || threshold > 100) return;
        if (pulseDurationUs < 1 || pulseDurationUs > 300) return;
        await TciClient.SendMessageAsync($"{TciRxNbParamCommand.Name}:{transceiverPeriodicNumber},{threshold},{pulseDurationUs};");
    }

    public async Task RxBinEnable(uint transceiverPeriodicNumber, bool state)
    {
        var trx = GetTransceiver(transceiverPeriodicNumber);
        if (trx == null || trx.RxBin == state) return;
        await TciClient.SendMessageAsync($"{TciRxBinEnableCommand.Name}:{transceiverPeriodicNumber},{(state ? "true" : "false")};");
    }

    public async Task RxNrEnable(uint transceiverPeriodicNumber, bool state)
    {
        var trx = GetTransceiver(transceiverPeriodicNumber);
        if (trx == null || trx.RxNr == state) return;
        await TciClient.SendMessageAsync($"{TciRxNrEnableCommand.Name}:{transceiverPeriodicNumber},{(state ? "true" : "false")};");
    }

    public async Task RxAncEnable(uint transceiverPeriodicNumber, bool state)
    {
        var trx = GetTransceiver(transceiverPeriodicNumber);
        if (trx == null || trx.RxAnc == state) return;
        await TciClient.SendMessageAsync($"{TciRxAncEnableCommand.Name}:{transceiverPeriodicNumber},{(state ? "true" : "false")};");
    }

    public async Task RxAnfEnable(uint transceiverPeriodicNumber, bool state)
    {
        var trx = GetTransceiver(transceiverPeriodicNumber);
        if (trx == null || trx.RxAnf == state) return;
        await TciClient.SendMessageAsync($"{TciRxAnfEnableCommand.Name}:{transceiverPeriodicNumber},{(state ? "true" : "false")};");
    }

    public async Task RxApfEnable(uint transceiverPeriodicNumber, bool state)
    {
        var trx = GetTransceiver(transceiverPeriodicNumber);
        if (trx == null || trx.RxApf == state) return;
        await TciClient.SendMessageAsync($"{TciRxApfEnableCommand.Name}:{transceiverPeriodicNumber},{(state ? "true" : "false")};");
    }

    public async Task RxDseEnable(uint transceiverPeriodicNumber, bool state)
    {
        var trx = GetTransceiver(transceiverPeriodicNumber);
        if (trx == null || trx.RxDse == state) return;
        await TciClient.SendMessageAsync($"{TciRxDseEnableCommand.Name}:{transceiverPeriodicNumber},{(state ? "true" : "false")};");
    }

    public async Task RxNfEnable(uint transceiverPeriodicNumber, bool state)
    {
        var trx = GetTransceiver(transceiverPeriodicNumber);
        if (trx == null || trx.RxNf == state) return;
        await TciClient.SendMessageAsync($"{TciRxNfEnableCommand.Name}:{transceiverPeriodicNumber},{(state ? "true" : "false")};");
    }

    public async Task Lock(uint transceiverPeriodicNumber, bool state)
    {
        var trx = GetTransceiver(transceiverPeriodicNumber);
        if (trx == null || trx.Lock == state) return;
        await TciClient.SendMessageAsync($"{TciLockCommand.Name}:{transceiverPeriodicNumber},{(state ? "true" : "false")};");
    }

    public async Task VfoLock(uint transceiverPeriodicNumber, uint channelNumber, bool state)
    {
        var trx = GetTransceiver(transceiverPeriodicNumber);
        var ch = trx?.Channels.FirstOrDefault(c => c.PeriodicNumber == channelNumber);
        if (ch == null || ch.VfoLock == state) return;
        await TciClient.SendMessageAsync($"{TciVfoLockCommand.Name}:{transceiverPeriodicNumber},{channelNumber},{(state ? "true" : "false")};");
    }

    public async Task RxVolume(uint transceiverPeriodicNumber, uint channelNumber, int valueInDb)
    {
        if (valueInDb < -60 || valueInDb > 0) return;
        var trx = GetTransceiver(transceiverPeriodicNumber);
        var ch = trx?.Channels.FirstOrDefault(c => c.PeriodicNumber == channelNumber);
        if (ch == null || ch.RxVolume == valueInDb) return;
        await TciClient.SendMessageAsync($"{TciRxVolumeCommand.Name}:{transceiverPeriodicNumber},{channelNumber},{valueInDb};");
    }

    public async Task RxBalance(uint transceiverPeriodicNumber, uint channelNumber, int valueInDb)
    {
        if (valueInDb < -40 || valueInDb > 40) return;
        var trx = GetTransceiver(transceiverPeriodicNumber);
        var ch = trx?.Channels.FirstOrDefault(c => c.PeriodicNumber == channelNumber);
        if (ch == null || ch.RxBalance == valueInDb) return;
        await TciClient.SendMessageAsync($"{TciRxBalanceCommand.Name}:{transceiverPeriodicNumber},{channelNumber},{valueInDb};");
    }

    public async Task SetDiglOffset(int offsetHz)
    {
        if (offsetHz < 0 || offsetHz > 4000) return;
        if (DiglOffset == offsetHz) return;
        await TciClient.SendMessageAsync($"{TciDiglOffsetCommand.Name}:{offsetHz};");
    }

    public async Task SetDiguOffset(int offsetHz)
    {
        if (offsetHz < 0 || offsetHz > 4000) return;
        if (DiguOffset == offsetHz) return;
        await TciClient.SendMessageAsync($"{TciDiguOffsetCommand.Name}:{offsetHz};");
    }

    public async Task LineOutStart(uint receiverPeriodicNumber)
    {
        await TciClient.SendMessageAsync($"{TciLineOutStartCommand.Name}:{receiverPeriodicNumber};");
    }

    public async Task LineOutStop(uint receiverPeriodicNumber)
    {
        await TciClient.SendMessageAsync($"{TciLineOutStopCommand.Name}:{receiverPeriodicNumber};");
    }

    public async Task LineOutRecorderStart(uint receiverPeriodicNumber, uint maxRecordingSeconds)
    {
        if (maxRecordingSeconds == 0 || maxRecordingSeconds > 300) return;
        await TciClient.SendMessageAsync($"{TciLineOutRecorderStartCommand.Name}:{receiverPeriodicNumber},{maxRecordingSeconds};");
    }

    public async Task LineOutRecorderSave(uint receiverPeriodicNumber, string filePath)
    {
        if (string.IsNullOrEmpty(filePath)) return;
        // Per spec: ":" in Windows paths must be replaced with "|".
        var safePath = filePath.Replace(":", "|");
        await TciClient.SendMessageAsync($"{TciLineOutRecorderSaveCommand.Name}:{receiverPeriodicNumber},{safePath};");
    }

    public async Task LineOutRecorderBreak(uint receiverPeriodicNumber)
    {
        await TciClient.SendMessageAsync($"{TciLineOutRecorderBreakCommand.Name}:{receiverPeriodicNumber};");
    }

    public async Task SetAudioStreamSampleType(string format)
    {
        if (format != "int16" && format != "int24" && format != "int32" && format != "float32") return;
        await TciClient.SendMessageAsync($"{TciAudioStreamSampleTypeCommand.Name}:{format};");
    }

    public async Task SetAudioStreamChannels(uint channels)
    {
        if (channels != 1 && channels != 2) return;
        await TciClient.SendMessageAsync($"{TciAudioStreamChannelsCommand.Name}:{channels};");
    }

    public async Task SetAudioStreamSamples(uint samples)
    {
        if (samples < 100 || samples > 2048) return;
        await TciClient.SendMessageAsync($"{TciAudioStreamSamplesCommand.Name}:{samples};");
    }

    public async Task SetTxStreamAudioBuffering(uint timeoutMs)
    {
        if (timeoutMs < 50 || timeoutMs > 500) return;
        await TciClient.SendMessageAsync($"{TciTxStreamAudioBufferingCommand.Name}:{timeoutMs};");
    }

    public async Task SetInFocus()
    {
        await TciClient.SendMessageAsync($"{TciSetInFocusCommand.Name};");
    }

    public async Task SetRxSensorsEnable(bool state, uint sendingIntervalMs = 0)
    {
        if (sendingIntervalMs == 0)
        {
            await TciClient.SendMessageAsync($"{TciRxSensorsEnableCommand.Name}:{(state ? "true" : "false")};");
        }
        else
        {
            await TciClient.SendMessageAsync($"{TciRxSensorsEnableCommand.Name}:{(state ? "true" : "false")},{sendingIntervalMs};");
        }
    }

    public async Task SetTxSensorsEnable(bool state, uint sendingIntervalMs = 0)
    {
        if (sendingIntervalMs == 0)
        {
            await TciClient.SendMessageAsync($"{TciTxSensorsEnableCommand.Name}:{(state ? "true" : "false")};");
        }
        else
        {
            await TciClient.SendMessageAsync($"{TciTxSensorsEnableCommand.Name}:{(state ? "true" : "false")},{sendingIntervalMs};");
        }
    }

    public async Task Keyer(uint transceiverPeriodicNumber, bool pressed, int previousCharacterDurationMs = 0)
    {
    ...
    /// <summary>Raises <see cref="OnSpotClicked"/> from a command parser.</summary>
    internal void RaiseSpotClicked(SpotClickedEventArgs args) => OnSpotClicked?.Invoke(this, args);

    /// <summary>Raises <see cref="OnRxSpotClicked"/> from a command parser.</summary>
    internal void RaiseRxSpotClicked(RxSpotClickedEventArgs args) => OnRxSpotClicked?.Invoke(this, args);

    /// <summary>Raises <see cref="OnKeyer"/> from a command parser.</summary>
    internal void RaiseKeyer(KeyerEventArgs args) => OnKeyer?.Invoke(this, args);

    /// <summary>Raises <see cref="OnTxSensorsChanged"/> from a command parser.</summary>
    internal void RaiseTxSensors(TxSensorsEventArgs args) => OnTxSensorsChanged?.Invoke(this, args);
    ...
    public event EventHandler<UintValueChangedEventArgs> OnCwKeyerSpeedChanged;
    public event EventHandler<IntValueChangedEventArgs> OnMonVolumeChanged;
    public event EventHandler<StateChangeEventArgs> OnMonEnableChanged;
    public event EventHandler<IntValueChangedEventArgs> OnDiglOffsetChanged;
    public event EventHandler<IntValueChangedEventArgs> OnDiguOffsetChanged;
    public event EventHandler<StringValueChangedEventArgs> OnAudioStreamSampleTypeChanged;
    public event EventHandler<UintValueChangedEventArgs> OnAudioStreamChannelsChanged;
    public event EventHandler<UintValueChangedEventArgs> OnAudioStreamSamplesChanged;
    public event EventHandler<UintValueChangedEventArgs> OnTxStreamAudioBufferingChanged;
    public event EventHandler<StateChangeEventArgs> OnRxSensorsEnableChanged;
    public event EventHandler<StateChangeEventArgs> OnTxSensorsEnableChanged;
    public event EventHandler<StateChangeEventArgs> OnAppFocusChanged;
    public event EventHandler<LongValueChangedEventArgs> OnTxFrequencyChanged;
    public event EventHandler<SpotClickedEventArgs> OnSpotClicked;
    public event EventHandler<RxSpotClickedEventArgs> OnRxSpotClicked;
    ...
    private uint _cwKeyerSpeed;
    private int _monVolume;
    private bool _monEnable;
    private int _diglOffset;
    private int _diguOffset;
    private string _audioStreamSampleType;
    private uint _audioStreamChannels;
    private uint _audioStreamSamples;
    private uint _txStreamAudioBuffering;
    private bool _rxSensorsEnable;
    private bool _txSensorsEnable;
    private bool _appFocus;
    private long _txFrequency;
